...
let widthInput = null;
let dashSelect = null;
let bounceSection = null;
let bounceColorInput = null;
let bounceWidthInput = null;
let bounceDashSelect = null;
let titleEl = null;
...
/**
 * Helper to create a dash <select> element
 */
function createDashSelect() {
    const select = document.createElement('select');
    select.className = 'w-full px-3 py-2 lg:px-2 lg:py-1 text-sm border-2 border-gray-300 rounded focus:outline-none focus:border-[#6ad1e3] transition-colors bg-white';
...
function createModal() {
    ...
    modalOverlay.id = 'cel-line-editor-overlay';
    ...
    // --- Bounce section (conditionally shown) ---
    bounceSection = document.createElement('div');
    bounceSection.style.display = 'none';

    const bounceDivider = document.createElement('div');
    bounceDivider.className = 'border-t border-gray-200 my-4';

    const bounceSectionLabel = document.createElement('div');
    bounceSectionLabel.className = 'text-sm font-semibold text-gray-600 mb-3 text-center';
    bounceSectionLabel.textContent = 'Bounce Envelope';

    // Bounce Color
    const bounceColorRow = document.createElement('div');
    bounceColorRow.className = 'mb-3';

    const bounceColorLabel = document.createElement('label');
    bounceColorLabel.className = 'block text-sm text-gray-500 mb-1';
    bounceColorLabel.textContent = 'Color';
    ...
    bounceColorRow.appendChild(bounceColorLabel);
    bounceColorRow.appendChild(bounceColorInput);

    // Bounce Width
    const bounceWidthRow = document.createElement('div');
    bounceWidthRow.className = 'mb-3';

    const bounceWidthLabel = document.createElement('label');
    bounceWidthLabel.className = 'block text-sm text-gray-500 mb-1';
    bounceWidthLabel.textContent = 'Width';
    ...
    bounceWidthRow.appendChild(bounceWidthLabel);
    bounceWidthRow.appendChild(bounceWidthInput);

    // Bounce Dash
    const bounceDashRow = document.createElement('div');
    bounceDashRow.className = 'mb-4';

    const bounceDashLabel = document.createElement('label');
    bounceDashLabel.className = 'block text-sm text-gray-500 mb-1';
    bounceDashLabel.textContent = 'Dash Style';

    bounceDashSelect = createDashSelect();
    ...
    bounceDashRow.appendChild(bounceDashLabel);
    bounceDashRow.appendChild(bounceDashSelect);

    bounceSection.appendChild(bounceDivider);
    bounceSection.appendChild(bounceSectionLabel);
    bounceSection.appendChild(bounceColorRow);
    bounceSection.appendChild(bounceWidthRow);
    bounceSection.appendChild(bounceDashRow);
    ...
    content.appendChild(bounceSection);
...
function getActiveStyle() {
    ...
    const metadata = chartState.CelLines[activeLineId];
    if (!metadata) return null;
    return metadata.style;
}

/**
 * Check if a line has bounce envelope data
 */
function hasBounceLines(metadata) {
    return metadata.bounceUpperY1 != null || metadata.bounceLowerY1 != null;
}

/**
 * Show the per-line style editor
 * @param {number} lineId - The CelLines key to edit
 */
export function showCelLineEditor(lineId) {
    ...
    // Set title from line metadata
    titleEl.textContent = `Edit: ${metadata.text}`;

    // Sync main style controls
    const style = metadata.style;
    colorInput.value = style.color;
    widthInput.value = style.width;
    dashSelect.value = style.dash;

    // Show/hide bounce section based on whether this line has bounce data
    if (hasBounceLines(metadata)) {
        bounceSection.style.display = '';
...
function hideModal() {
    ...
    // Trigger cel line redraw to reflect style changes
    if (activeLineId != null) {
        eventBus.emit(EVENTS.LINE_CEL_STYLE_CHANGED, { lineId: activeLineId });
        activeLineId = null;
    }
}
