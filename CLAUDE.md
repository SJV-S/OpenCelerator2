...
**Tailwind is pre-built, not a CDN.** The CSS at `static/SCC/css/tailwind.css` is generated by a standalone Tailwind CLI. It only contains classes actually used in the scanned files. **After adding any new Tailwind classes**, you MUST rebuild by running:
```
./scripts/build/Tailwind/build.sh
```
If you skip this step, new classes will have no effect and the page will look broken. The config is at `scripts/build/Tailwind/tailwind.config.js` and scans `templates/**/*.html` + `static/SCC/**/*.js` + `static/Server/**/*.js`.
