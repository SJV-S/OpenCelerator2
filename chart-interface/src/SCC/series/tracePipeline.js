 ...
 *
 * When onXAgg='raw' (TC2 default), each session's shadow is computed against
 * its own timing floor and is always below it by construction. When onXAgg='median'
 * (FL-Flash2), multiple sessions per day can have different timings: below-floor
 * sessions contribute to the shadow median while all sessions contribute to the
 * timing floor median, so the shadow can end up above the floor. correctFloorY
 * fixes this by anchoring shadow y-values to timing_agg * FLOOR_MULTIPLIER
 * post-aggregation, guaranteeing the shadow is always below the rendered floor line.
 */
function createFloorShadowTraces(xPositions, frequencies) {
    const floorShadowTraces = [];

    const rawTimingForFloor = chartState.minuteChart
        ? timingToFrequency(chartState.series.timing)
        : chartState.series.timing.map(() => 1);

    function correctFloorY(y, onXAgg) {
        const { y: timY } = applyAggregation(xPositions, rawTimingForFloor, onXAgg);
        return y.map((val, i) => {
            if (val === null || val === undefined || isNaN(val)) return null;
            const tf = timY[i];
            return (tf != null && !isNaN(tf) && tf > 0) ? tf * CHART_MATH.FLOOR_MULTIPLIER : val;
        });
    }
    ...
    if (hasValidData(frequencies.correctsFloor)) {
        Object.entries(chartState.traceStyles[CORRECTS]).forEach(([aggId, config]) => {
            ...
            const trace = createFloorShadowTrace(correctsTrace, x, correctFloorY(y, onXAgg), config);
    ...
    if (hasValidData(frequencies.errorsFloor)) {
        Object.entries(chartState.traceStyles[ERRORS]).forEach(([aggId, config]) => {
            ...
            const trace = createFloorShadowTrace(errorTrace, x, correctFloorY(y, onXAgg), config);
    ...
    Object.entries(chartState.traceStyles.misc).forEach(([miscId, aggConfigs]) => {
        Object.entries(aggConfigs).forEach(([aggId, config]) => {
            ...
            const trace = createFloorShadowTrace(miscTrace, x, correctFloorY(y, onXAgg), config);
