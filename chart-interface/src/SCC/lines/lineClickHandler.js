...
function setupClickHandler() {
    ...
    chartDiv.on('plotly_click', function(eventData) {
        ...
        console.log('[lineClick] plotly_click fired, points:', points.length,
            points.map(p => ({ meta: p.data?.meta, name: p.data?.name, curve: p.curveNumber })));
...
function setLineCategoryClickability(category, makeClickable) {
    ...
    console.log('[lineClick] setLineCategoryClickability', category, makeClickable,
        'chartDiv.data.length:', chartDiv.data.length);
    ...
    if (makeClickable) {
        ...
        clickablePromise.then(() => {
            ...
            console.log('[lineClick] clickable traces added, indices:', clickableIndices,
                'total traces:', chartDiv.data.length);
...
function init() {
    ...
    // After a replot, react() wipes all clickable overlay traces.
    // Re-enable any category that was in edit mode before the replot.
    // Must use PLOTLY_REACT_COMPLETE (not DATA_CHART_REPLOT_COMPLETE) because
    // DATA_CHART_REPLOT_COMPLETE fires synchronously before Plotly.react() finishes —
    // addTraces() would race against react()'s internal rendering and lose.
    eventBus.subscribe(EVENTS.PLOTLY_REACT_COMPLETE, () => {
        for (const [category, active] of Object.entries(categoryEditState)) {
            if (active) {
                setLineCategoryClickability(category, true);
            }
        }
    });
